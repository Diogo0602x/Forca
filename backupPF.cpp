...
int main(){
	...
	logo_forca();
	getchar();
	int opcao_menu;			//Menu de op��es
		do{
			opcao_menu = menu();		//Fun��o menu
			switch(opcao_menu){
				case 0:
					clrscr();
					getchar();
					...
					desenho_forca();
					logo_forca();
					gotoxy(7,3);textcolor(7);printf("Saindo... \n");		//Mensagem ao escolher a op��o para fechar o console
					gotoxy(6,26);
					break;
				case 1:
					do{
						...
						getchar();
						desenho_quadro();
						banco_palavras();		//Fun��o para escolher a palavra secreta
						resetar_variaveis();	//Gambiarra para resetar os dados das variaveis
						do{
							jogo_forca();		//Chamada de fun��es necess�rias para o jogo
							
							chutes();			//Chamada de fun��es necess�rias para controlar os chutes
...
void jogo_forca(){
	...
	//FORCA LINHA HORIZONTAL MAIOR(CH�O)
	for(int i =47; i<61; i++){		
...
int letras_chutadas(char letra){		//desenha forca // Letras J� chutou
	int achou = 0;
...
void bem_vindo(){
	...
	gotoxy(7,4);textcolor(7);printf("Autor: Diogo Sales\n");
}
